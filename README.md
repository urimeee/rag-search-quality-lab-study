...
청킹 전략·검색 파라미터가 검색 품질에 미치는 영향을 실험할 수 있는 플랫폼입니다.

---

## 프로젝트 개요

이 프로젝트는 단순한 AI 면접 도구가 아니라, **RAG 파이프라인의 핵심 파라미터를 직접 조작하고 결과를 관찰**하는 실험 환경입니다.

- Chunk Size / Overlap / Top-K 조정에 따른 검색 결과 변화를 시각적으로 비교
- 검색된 청크의 유사도 점수를 기반으로 Retrieval 품질 분석
- Hallucination 발생 원인(Retrieval 실패 vs Generation 실패)을 구분하는 프레임워크 제공
- 이력서와 JD(채용공고)를 결합한 맞춤형 AI 모의 면접 기능
...
- 이력서와 JD를 RAG로 결합하여 맞춤형 질문 5개 자동 생성
- 답변 평가 후 꼬리 질문 또는 다음 질문으로 자동 진행
- 면접 완료 후 강점·개선점·준비 방향 종합 피드백
- 전체 면접 내용 텍스트 파일 다운로드
...
└── rag/
    ...
    ├── interviewer.py      # 질문 생성, 답변 평가, 피드백
    └── prompts.py          # 프롬프트 템플릿
```
...
- OpenAI API 사용 비용이 발생합니다 (이력서 임베딩 1회 약 $0.001, 면접 1회 약 $0.05~0.10).
